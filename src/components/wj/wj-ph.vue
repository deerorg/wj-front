...
export default {
    ...
    methods:{
        beforeanswer() {
            if(getUserAgent() === '3') {
                    this.$router.push(`/wjforanswer/pc:${getIdfromUrl()}?crid=${getCreatUserId()}`)
            } else {
                this.channel = getUserAgent()
            }
            if(getToken()) {
            ...
            if(!this.validataSingleQ()) {
                ...
                for(let i = 0; i < this.answer.length; i++) {
                   ...
                   // console.log(this.wj.testList[this.answer[i].testIndex])
                    ...
                    if(key) {
                        let val = this.wj.testList[this.answer[i].testIndex].optionList.map(x => {
